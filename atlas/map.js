...
function renderSitePanel(p, l, state){
  ...
  h += `<div class="sp-actions"><button class="btn" onclick="openDashboard(${p.idx})">Technical results →</button><button class="btn ghost" title="This run's stores, the storage-technology sweep and the cell lab" onclick="openStorageTab(${p.idx})">Energy storage →</button><button class="btn ghost" onclick="FAC.on?hideFacility():selectRun(siteRun.path,siteRun.policy,siteRun.ci)">${(typeof FAC !== 'undefined' && FAC.on) ? 'Hide plant' : 'Show plant'}</button>${state !== 'none' ? `<button class="btn ghost" onclick="siteRenewables(${!renewOn})">${renewOn ? 'Hide renewables' : 'Show renewables'}</button>` : ''}</div>`;
...
async function ensureResourceGrid(){
  ...
  map.addLayer({ id: 'resgrid-fill', type: 'fill', source: 'resgrid', layout: { visibility: 'none' }, filter: ['==', ['get', 'land'], 1], paint: { 'fill-color': '#000', 'fill-antialias': false, 'fill-opacity': ['interpolate', ['linear'], ['zoom'], 2, .88, 8, .7, 11, .35] } }, 'hops-halo');
  map.on('mousemove', 'resgrid-fill', e => { if (!RES.metric) return; const p = e.features[0].properties, tip = document.getElementById('tip'); tip.style.opacity = 1; tip.style.left = (e.originalEvent.clientX + 14) + 'px'; tip.style.top = (e.originalEvent.clientY + 14) + 'px';
    tip.innerHTML = `<div class="t-n">${p.lat.toFixed(2)}°, ${p.lon.toFixed(2)}° · ${p.region}</div><div class="t-m">solar CF ${gridFmt('solar', p.solar)} (${fmt(p.solar * 8760)} full-load h) · wind CF ${gridFmt('wind', p.wind)} (${fmt(p.wind * 8760)} h)</div><div class="t-cta">${(typeof BUILD !== 'undefined' && BUILD.on) ? 'click to build here' : 'Build a plant → to use this cell'}</div>`; });
