...
function buildStorage(keepDuty){
  ...
  h += `<div class="section-t">Zoom in: the battery, one cell at a time</div><p class="section-d">A bottom-up Li-ion cell: chemistry, format and electrode design → capacity, energy density, mass and materials — multiplied out to the ${fmt(batMWh)} MWh battery this run builds.</p><div id="cellLabHost"></div>`;
  host.innerHTML = h;
  renderCellLab(document.getElementById('cellLabHost'), { batMWh, pMW: r.p_b, plantName: plant.name, ci: r.target, capex_b: r.capex_b });
}
